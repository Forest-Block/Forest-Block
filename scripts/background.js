...
chrome.runtime.onMessage.addListener(function(request, sender, sendResponse)
{
    ...
    if (request.message=="transfer"){
        //sendResponse('我是background，我已收到你的消息：' + JSON.stringify(request));
        //console.log('收到来自content-script的消息：transfer');
        
         chrome.tabs.update({url: chrome.extension.getURL('bg.html')})
    }
});
//2 关键词存储
...
/*chrome.storage.sync.get({blockWords: []}, function(items) {
        console.log(items.blockWords);
        if (!items.blockWords.length){
          // 保存数据
        chrome.storage.sync.set({blockWords: init_badWordList}, function() {
                badWordList=init_badWordList
                console.log('保存成功！');
        });
        }else{
        badWordList=items.blockWords
        }
